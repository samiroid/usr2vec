...
def parallel_extract(i, instance, unigram_distribution,negative_sample_size):        
    train = instance[1]    
    neg_samples = []
    sys.stdout.write("\ri:%d " % i)
    sys.stdout.flush()    
    for msg in train:            
        neg_samp = [negative_samples(unigram_distribution, msg, negative_sample_size) for _ in xrange(len(msg))]        
...
if __name__ == "__main__":
    ...
    training_data_path, aux_data, negative_sample_size, n_jobs = sys.argv[1:]
    n_jobs = int(n_jobs)
    negative_sample_size = int(negative_sample_size)
    print "Loading data"    
    with open(aux_data,"r") as fid:
    ...
    while not done:        
        ...
        if n_jobs>1:
            res = Parallel(n_jobs=n_jobs)(delayed(parallel_extract)(i, instance, unigram_distribution, negative_sample_size) for i, instance in enumerate(current_instances))
        else:
